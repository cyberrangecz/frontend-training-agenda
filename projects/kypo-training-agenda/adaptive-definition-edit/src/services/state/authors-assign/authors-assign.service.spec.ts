...
import { OffsetPagination, PaginatedResource, asyncData, OffsetPaginationEvent } from '@sentinel/common';
...
describe('AuthorsAssignService', () => {
  ...
  it('should assign designers through facade (called once)', (done) => {
    ...
    expect(apiSpy.updateAuthors).toHaveBeenCalledWith(0, idsToAssign, true, []);
  });
  ...
  it('should unassign authors through facade (called once)', (done) => {
    ...
    expect(apiSpy.updateAuthors).toHaveBeenCalledTimes(1);
    expect(apiSpy.updateAuthors).toHaveBeenCalledWith(0, [], true, idsToUnassign);
  });
